...
public class ExercicioMap extends AppCompatActivity {
    private MapView map;
    private ArrayList<String> listaLat, listaLong;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_exercicio_map);

        Intent intent = getIntent();
        listaLat = intent.getStringArrayListExtra("lat");
        listaLong = intent.getStringArrayListExtra("long");
    ...
    public void createmarker() {
        ...
        for (int i = 0; i < listaLat.size(); i++) {
            GeoPoint gp = new GeoPoint(Double.parseDouble(listaLat.get(i)), Double.parseDouble(listaLong.get(i)));
